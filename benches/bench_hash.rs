...
fn bench_data_hash_1kb(c: &mut Criterion) {
    ...
    c.bench_function("data_hash_1kb", |b| {
...
fn bench_data_hash_64kb(c: &mut Criterion) {
    ...
    c.bench_function("data_hash_64kb", |b| {
        b.iter(|| compute_data_hash(&data))
    });
}
...
fn bench_xorb_hash_1024_chunks(c: &mut Criterion) {
    ...
    c.bench_function("xorb_hash_1024", |b| {
        b.iter(|| xorb_hash(&chunks))
    });
}

criterion_group!(benches, bench_data_hash_1kb, bench_data_hash_64kb, bench_xorb_hash_1024_chunks);
