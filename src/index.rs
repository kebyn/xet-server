...
impl MetadataIndex {
    ...
    pub fn register_shard(
        ...
        {
            let mut file_map = self.file_to_shards.write().unwrap();
        ...
        {
            let mut chunk_map = self.chunk_to_xorb.write().unwrap();
    ...
    pub fn get_shards_for_file(&self, file_hash: &str) -> Option<Vec<String>> {
        let file_map = self.file_to_shards.read().unwrap();
    ...
    pub fn get_xorb_for_chunk(&self, chunk_hash: &str) -> Option<(String, u32)> {
        let chunk_map = self.chunk_to_xorb.read().unwrap();
