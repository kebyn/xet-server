...
Xet Server 使用**分层认证系统**：Hub API 使用基于 SQLite 的令牌存储，CAS 使用 Ed25519 签名的 JWT。
...
└──────────┬──────────────────────────────────┬───────────┘
           │                                  │
           │ 1. Hub Token (hf_xxx)           │ 4. CAS user/proxy/internal token
           │    不透明令牌（UUID）             │    (xet_xxx / proxy_xxx / internal_xxx)
           │                                  │
           ▼                                  ▼
┌──────────────────────┐          ┌──────────────────────┐
...
- 当前 Hub LFS 对象字节路径是 content-hash capability：batch 阶段按 Hub token scope 授权并签发绑定 OID/operation 的 proxy token；对象上传/下载阶段校验 proxy token，不校验 OID 是否属于 URL 中的 repo。私有 repo 的 OID 由 tree、resolve 和 repo metadata API 保护。
...
| `internal` | 内部权限 | Hub → CAS 内部端点（defense-in-depth：同时验证 sub=hub-service + scope=internal + token_type=internal） |
...
3. Hub API 调用 CAS batch，再重写 action URL 和 Proxy Token
   - 使用短期 `xet_xxx` user token 调用 CAS batch API（`download` 使用 `read` scope，`upload` 使用 `write` scope）
   - 使用 Hub 私钥创建 proxy JWT claims (绑定到 oid 和 operation)
   - 返回 Hub `/lfs/objects/{oid}` action URL 和 proxy_xxx 令牌

4. 客户端 → Hub API
   PUT /lfs/objects/{oid}
   Authorization: Bearer proxy_eyJhbGci...

5. Hub API 验证 Proxy Token
   - 验证 Ed25519 签名
   - 检查 oid 是否匹配
   - 检查 operation 是否匹配
   - 检查 scope 是否为 lfs-upload
   - 不校验 OID 是否属于 URL 中的 repo

6. Hub API 将同一个 `proxy_xxx` token 转发给 CAS Server 的 `/lfs/objects/{oid}` 公共端点
...
**Hub 内部调用公共 CAS 对象端点**：Hub 的 commit inline 上传和 resolve inline 直读会调用 CAS `/lfs/objects/{oid}` 公共端点，因此使用短期 `xet_xxx` user token（分别为 `write` / `read` scope）。`internal_xxx` 只用于 `/internal/*` 这类内部服务端点。
...
- CAS 私钥（`CAS_PRIVATE_KEY_PATH`）用于在 Batch API 中签发短期 proxy token
- **生产环境强烈建议配置**：设置后，Batch API 会签发 5 分钟有效的 proxy token，绑定特定 OID 和操作，避免用户长期 token 泄露到客户端
- **未配置时的风险**：Batch API 会回退到直接传递用户的长期 CAS token（1小时有效期）给客户端，增加 token 泄露风险
- 服务启动时会检查私钥文件权限，若 group/other 可读会输出安全警告
