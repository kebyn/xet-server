...
- **速率限制**：60 requests/minute per IP
...
- 转换过程会加载整个文件到内存进行 CDC 分块
- `XET_MAX_CONVERSION_SIZE` 用于防止大文件导致 OOM
...
2. **清理流程**：
   ```
   定时触发 → 查询 Hub 获取引用哈希 → 对比存储中的 blob → 删除未引用 blob（应用宽限期）
   ```
...
- **CAS Server**：
  - 60 requests/minute per IP（公共端点）
  - 内部端点（`/internal/*`）绕过限制
  
- **Hub API**：
  - 120 requests/minute per IP（公共端点）
  - 内部端点绕过限制
...
将 MetadataIndex 保存到 SQLite，避免每次重启时扫描所有 shard 重建索引。

### 工作原理

1. **启用持久化**（`XET_INDEX_PERSIST=true`）：
   - 启动时从 SQLite 数据库加载索引
   - 运行期间内存中维护索引
   - 关闭时将索引保存回数据库

2. **禁用持久化**（默认）：
   - 每次启动扫描存储后端中的所有 shard 文件
   - 解析 shard 元数据重建内存索引
   - 启动时间随 shard 数量增加而增加
...
| 环境变量 | 描述 | 默认值 |
|---------|------|--------|
| `XET_INDEX_PERSIST` | 启用索引持久化 | `false` |
| `XET_INDEX_DB_PATH` | SQLite 数据库路径 | `./data/metadata_index.db` |

### 性能考虑

- 对于大型部署（数万个 shard），启用持久化可以显著减少启动时间
- 数据库文件建议使用 SSD 存储以获得最佳性能
- 索引持久化是原子操作，不会因中断导致索引损坏
- 首次启用时会扫描所有 shard 并创建数据库，后续启动直接加载
