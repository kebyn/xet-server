...
- 管理仓库、版本、文件树元数据
- 签发 CAS user token（xet_xxx）、LFS proxy token（proxy_xxx）和 internal service token（internal_xxx）
- 代理 LFS 请求到 CAS
- 提供 `/health` 存活检查和 `/ready` 就绪检查；就绪检查会验证 SQLite 与 CAS `/ready`
...
- **就绪状态**：`/health` 仅表示进程存活，`/ready` 会验证存储后端可访问且 `MetadataIndex` 已完成重建
...
│   ├── xorb.rs       # Xorb 上传/下载
│   ├── shard.rs      # Shard 上传
│   ├── lfs.rs        # LFS 对象 handler facade
│   ├── lfs/          # LFS raw download / verified reconstruction helper
...
### 2026-07-06 就绪检查与配置解析收紧

**新增配置项**：
- `XET_INDEX_REBUILD_STRICT` (false) - CAS MetadataIndex 重建失败时是否启动失败

**探针语义**：
- `/health` 是 liveness probe，只表示 HTTP server 存活
- `/ready` 是 readiness probe；CAS 检查存储后端和索引状态，Hub 检查 SQLite 和 CAS `/ready`

**配置解析行为**：
- 显式设置的数值和布尔环境变量解析失败时启动失败，不再静默回退默认值
